...
php5.4の [] が便利すぐる。
php5.3 以上じゃないとラムダ使えないし・・・

php5.2はアクティブサポートが終了しているようだし、まぁいいんじゃなイカ。
...
function perl_is_hash(array $array) {
    ...
    //そうでなければ、ハッシュとみなす.
    $count = 0;
    foreach(perl_sort(array_keys($array)) as $n) {
        if ($n !== $count) {
           break;
        }
        $count ++;
...
//            return [ { '' => undef, $list->[0] => $list } ];
            return array( '__@UNDEF@__'=> 0 , $list[0] => $list );
//        }
        }
//        else {
        else {
...
//                            $new = { '' => undef, $token => [$token] };
                            $new = array( '__@UNDEF@__' => 0 , $token => array($token) );
//                        }
                        }
//                    }
                    }
...
//////////// このendは上のスコープにも同盟の変数がありますので end2 とします。
                if ( is_array($opt_path[0]) ) {
                     $end2 = array( '__@UNDEF@__' => 0, '__OPT_PATH@reference__' => $opt_path );
                }
                else {
                     $end2 = array( '__@UNDEF@__' => 0, $opt_path[0] => $opt_path );
                }
...
//        next unless defined $in->{$path};
        ...
        //PHPの場合ないときはエラーになるので、補正する.
        $_temp_path0 = isset($in[$path][0]) ? $in[$path][0] : 0;
